...
namespace Boids {
    [RequireMatchingQueriesForUpdate, UpdateInGroup(typeof(SimulationSystemGroup)), UpdateBefore(typeof(TransformSystemGroup))]
    public partial struct BoidSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var boidSettings in uniqueBoidTypes) {
                ...
                var cellObstaclePositionIndex =
                    CollectionHelper.CreateNativeArray<int, RewindableAllocator>(boidCount, ref world.UpdateAllocator);
                var cellTargetPositionIndex =
                    CollectionHelper.CreateNativeArray<int, RewindableAllocator>(boidCount, ref world.UpdateAllocator);
                var cellCount = CollectionHelper.CreateNativeArray<int, RewindableAllocator>(boidCount, ref world.UpdateAllocator);
                var cellObstacleDistance =
                    CollectionHelper.CreateNativeArray<float, RewindableAllocator>(boidCount, ref world.UpdateAllocator);
                ...
                var copyTargetPositions =
                    CollectionHelper.CreateNativeArray<float3, RewindableAllocator>(targetCount, ref world.UpdateAllocator);
                var copyObstaclePositions =
                    CollectionHelper.CreateNativeArray<float3, RewindableAllocator>(obstacleCount, ref world.UpdateAllocator);
                ...
                var initialTargetJob = new InitialPerTargetJob { TargetPositions = copyTargetPositions };
                var initialTargetJobHandle = initialTargetJob.ScheduleParallel(targetQuery, state.Dependency);

                var initialObstacleJob = new InitialPerObstacleJob { ObstaclePositions = copyObstaclePositions };
        ...
        partial struct SteerBoidJob : IJobEntity {
            ...
            void Execute([EntityIndexInQuery] int entityIndexInQuery, ref LocalToWorld localToWorld) {
                ...
                var alignmentResult = CurrentBoidVariant.AlignmentWeight * math.normalizesafe(alignment / neighborCount - forward);
                ...
                var targetHeading = CurrentBoidVariant.TargetWeight * math.normalizesafe(nearestTargetPosition - currentPosition);
                ...
                var avoidObstacleHeading = nearestObstaclePosition
                    + math.normalizesafe(obstacleSteering) * CurrentBoidVariant.ObstacleAversionDistance
                    - currentPosition;
