...
namespace Tutorials.Jobs.Step4 {
    public class FindNearest : MonoBehaviour {
        ...
        public void Update() {
            for (var i = 0; i < TargetPositions.Length; i++) { TargetPositions[i] = Spawner.TargetTransforms[i].localPosition; }

            for (var i = 0; i < SeekerPositions.Length; i++) { SeekerPositions[i] = Spawner.SeekerTransforms[i].localPosition; }

            var sortJob = TargetPositions.SortJob(new AxisXComparer { });
            ...
            var findHandle = findJob.Schedule(SeekerPositions.Length, 100, sortHandle);
            findHandle.Complete();

            for (var i = 0; i < SeekerPositions.Length; i++) { Debug.DrawLine(SeekerPositions[i], NearestTargetPositions[i]); }
        }
    }
}
