    ...
    public partial struct TornadoParticleJob : IJobEntity {
        ...
        public void Execute(ref LocalTransform transform, in Particle particle) {
            var tornadoPos = new float3(
                Tornado.x + BuildingSystem.TornadoSway(transform.Position.y, ElapsedTime),
            ...
            delta /= dist;
            var inForce = dist - math.saturate(tornadoPos.y / 50f) * 30f * particle.radiusMult + 2f;
            transform.Position += new float3(
                    -delta.z * ParticleSpinRate + delta.x * inForce,
                    ParticleUpwardSpeed,
                    delta.x * ParticleSpinRate + delta.z * inForce
                )
                * DeltaTime;
            if (transform.Position.y > 50f) { transform.Position.y = 0f; }
        }
    }
}
