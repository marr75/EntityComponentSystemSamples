    ...
    public class ExecuteAuthoring : MonoBehaviour {
        ...
        class Baker : Baker<ExecuteAuthoring> {
            public override void Bake(ExecuteAuthoring authoring) {
                ...
                if (authoring.ExecuteHeatSystem) { AddComponent<ExecuteHeat>(entity); }

                if (authoring.ExecuteBotSystem) { AddComponent<ExecuteBot>(entity); }

                if (authoring.ExecuteBucketSystem) { AddComponent<ExecuteBucket>(entity); }

                if (authoring.ExecuteTeamSystem) { AddComponent<ExecuteTeam>(entity); }

                if (authoring.ExecuteUISystem) { AddComponent<ExecuteUI>(entity); }

                if (authoring.ExecuteAnimationSystem) { AddComponent<ExecuteAnimation>(entity); }
            }
        }
    }
