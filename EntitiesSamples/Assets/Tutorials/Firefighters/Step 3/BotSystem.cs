...
namespace Tutorials.Firefighters {
    public partial struct BotSystem : ISystem {
        ...
        void BotUpdate_MainThread(
            ...
            foreach (var (bot, botTrans, botEntity) in SystemAPI.Query<RefRW<Bot>, RefRW<LocalTransform>>().WithEntityAccess()) {
                switch (bot.ValueRO.State) {
                    ...
                    case BotState.WAIT_IN_LINE: {
                        if (!bot.ValueRO.IsCarrying) { break; }

                        if (bot.ValueRO.IsFiller) {
                            var bucket = SystemAPI.GetComponentRW<Bucket>(bot.ValueRO.Bucket);
                            if (bucket.ValueRO.Water < 1) { bot.ValueRW.State = BotState.FILL_BUCKET; }
                            else // full bucket
                            {
                                bot.ValueRW.State = BotState.PASS_BUCKET;
                            }
                        ...
                        if (bot.ValueRO.IsDouser) {
                            var bucket = SystemAPI.GetComponentRW<Bucket>(bot.ValueRO.Bucket);
                            if (bucket.ValueRO.Water > 0) { bot.ValueRW.State = BotState.DOUSE_FIRE; }
                            else // empty bucket
                            {
                                bot.ValueRW.State = BotState.PASS_BUCKET;
                            }
        ...
        static bool MoveToTarget(ref LocalTransform botTrans, float2 targetPos, float moveSpeed) {
