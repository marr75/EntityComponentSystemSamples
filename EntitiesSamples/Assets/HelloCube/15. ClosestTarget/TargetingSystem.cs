...
namespace HelloCube.ClosestTarget {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            switch (spatialPartitioningType) {
                ...
                case SpatialPartitioningType.Simple: {
                    var positions = CollectionHelper.CreateNativeArray<PositionAndIndex>(
                        targetTransforms.Length,
                        state.WorldUpdateAllocator
                    );

                    for (var i = 0; i < positions.Length; i += 1) {
                        positions[i] = new PositionAndIndex { Index = i, Position = targetTransforms[i].Position.xz };
                    }
                ...
                case SpatialPartitioningType.KdTree: {
                    ...
                    // init KD tree
                    for (var i = 0; i < targetEntities.Length; i += 1) {
    ...
    public struct QueryKdTree : IJobChunk {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask) {
            ...
            for (var i = 0; i < chunk.Count; i++) {
                if (!Scratch.Neighbours.IsCreated) { Scratch.Neighbours = new NativePriorityHeap<KDTree.Neighbour>(1, Allocator.Temp); }

                Scratch.Neighbours.Clear();
                Tree.GetEntriesInRangeWithHeap(unfilteredChunkIndex, transforms[i].Position, float.MaxValue, ref Scratch.Neighbours);
    ...
    public partial struct NoPartitioning : IJobEntity {
        ...
        public void Execute(ref Target target, in LocalTransform translation) {
            ...
            for (var i = 0; i < TargetTransforms.Length; i += 1) {
    ...
    public struct AxisXComparer : IComparer<PositionAndIndex> {
        public int Compare(PositionAndIndex a, PositionAndIndex b) => a.Position.x.CompareTo(b.Position.x);
    }
