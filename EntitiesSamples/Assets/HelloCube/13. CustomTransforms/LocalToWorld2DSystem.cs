...
namespace HelloCube.CustomTransforms {
    ...
    [WorldSystemFilter(WorldSystemFilterFlags.Default | WorldSystemFilterFlags.Editor), UpdateInGroup(typeof(TransformSystemGroup)),
     UpdateAfter(typeof(ParentSystem)), BurstCompile]
    public partial struct LocalToWorld2DSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            var rootsQuery = SystemAPI.QueryBuilder().WithAll<LocalTransform2D>().WithAllRW<LocalToWorld>().WithNone<Parent>().Build();
        ...
        unsafe struct ComputeRootLocalToWorldJob : IJobChunk {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask) {
                ...
                var chunk2DLocalTransforms = (LocalTransform2D*)chunk.GetRequiredComponentDataPtrRO(ref LocalTransform2DTypeHandleRO);
                    ...
                    if (chunkPostTransformMatrices != null) {
                        for (int i = 0, chunkEntityCount = chunk.Count; i < chunkEntityCount; ++i) {
                            chunkLocalToWorlds[i].Value = math.mul(
                                chunk2DLocalTransforms[i].ToMatrix(),
        ...
        unsafe struct ComputeChildLocalToWorldJob : IJobChunk {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask) {
                ...
                var updateChildrenTransform = chunk.DidChange(ref ChildTypeHandle, LastSystemVersion);
                var chunkChildBuffers = chunk.GetBufferAccessor(ref ChildTypeHandle);
                updateChildrenTransform = updateChildrenTransform || chunk.DidChange(ref LocalToWorldTypeHandleRW, LastSystemVersion);
                var chunkLocalToWorlds = (LocalToWorld*)chunk.GetRequiredComponentDataPtrRO(ref LocalToWorldTypeHandleRW);
                for (int i = 0, chunkEntityCount = chunk.Count; i < chunkEntityCount; i++) {
