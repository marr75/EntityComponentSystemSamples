...
namespace HelloCube.StateChange {
    public partial struct SpinSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<ExecuteStateChange>(); }
    ...
    public partial struct SpinByValueJob : IJobEntity {
        ...
        void Execute(ref LocalTransform transform, in Spin spin) {
            if (spin.IsSpinning) { transform = transform.Rotate(Offset); }
        }
    }

    [WithAll(typeof(Spin)), BurstCompile]
    partial struct SpinJob : IJobEntity {
        public quaternion Offset;

        void Execute(ref LocalTransform transform) { transform = transform.Rotate(Offset); }
    }
}
