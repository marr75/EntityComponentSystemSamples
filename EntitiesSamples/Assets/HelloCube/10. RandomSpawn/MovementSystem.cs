...
namespace HelloCube.RandomSpawn {
    public partial struct MovementSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<ExecuteRandomSpawn>(); }
    ...
    [WithAll(typeof(Cube)), BurstCompile]
    public partial struct FallingCubeJob : IJobEntity {
        ...
        void Execute([ChunkIndexInQuery] int chunkIndex, Entity entity, ref LocalTransform cubeTransform) {
            ...
            if (cubeTransform.Position.y < 0) { ECB.DestroyEntity(chunkIndex, entity); }
        }
    }
}
