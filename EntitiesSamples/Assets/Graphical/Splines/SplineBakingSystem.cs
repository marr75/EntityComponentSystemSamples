...
namespace Graphical.Splines {
    ...
    public partial struct SplineBakingSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<ExecuteSplines>(); }

        [BurstCompile]
        public void OnUpdate(ref SystemState state) {
            var bakingOnlyEntities = new NativeList<Entity>(state.WorldUpdateAllocator);

            foreach (var (spline, splineBakingData, bufferEntity, bufferPosition) in SystemAPI
                .Query<RefRW<Spline>, RefRO<SplineBakingData>, DynamicBuffer<SplineControlPointEntity>,
                    DynamicBuffer<SplineControlPointPosition>>()) {
                bakingOnlyEntities.AddRange(bufferEntity.Reinterpret<Entity>().AsNativeArray());

                using var bakedPoints = BakePoints(
                ...
                for (var i = 0; i < bakedPoints.Length; i += 1) {
        ...
        NativeList<float3> BakePoints(Allocator allocator, NativeArray<float3> controlPoints, int subdivisions) {
            ...
            for (var i = 0; i < controlPoints.Length; i += 1) {
                var p0 = RingBufferAt(i - 1);
                var p1 = RingBufferAt(i);
                var p2 = RingBufferAt(i + 1);
                var p3 = RingBufferAt(i + 2);
