...
namespace Graphical.Splines {
    ...
    public partial struct SnakeUpdateSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            new SnakeJob { DeltaTime = SystemAPI.Time.DeltaTime, TransformLookup = transformLookup, SplineLookup = splineFromEntity }
                .ScheduleParallel();
        }
    }
    ...
    public partial struct SnakeJob : IJobEntity {
        ...
        void Execute(ref Snake snake, in DynamicBuffer<SnakePart> snakeParts) {
            ...
            for (var i = 0; i < snakeParts.Length; i += 1) {
                offset = (offset % maxDist + maxDist) % maxDist;

                var idx = distance.LowerBound(offset);
                var a = distance[idx];
                var b = distance[idx + 1];

                var t = (offset - a) / (b - a);
                var pos = math.lerp(points[idx], points[idx + 1], t);
