    ...
    public partial struct SnakeSpawnSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (snakeSettings, entity) in SystemAPI.Query<RefRO<SnakeSettings>>().WithEntityAccess()) {
                using (var snakes = state.EntityManager.CreateEntity(
                        snakeArchetype,
                        snakeSettings.ValueRO.NumSnakes,
                        state.WorldUpdateAllocator
                    )) {
                    var partsCount = snakeSettings.ValueRO.NumSnakes * snakeSettings.ValueRO.NumPartsPerSnake;
                    using (var snakeParts = state.EntityManager.Instantiate(
                            snakeSettings.ValueRO.Prefab,
                            partsCount,
                            state.WorldUpdateAllocator
                        )) {
                        for (var i = 0; i < snakes.Length; i += 1) {
                            ...
                            var slice = snakeParts.Slice(
                                i * snakeSettings.ValueRO.NumPartsPerSnake,
