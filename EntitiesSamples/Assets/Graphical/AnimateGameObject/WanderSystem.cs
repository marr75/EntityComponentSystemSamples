    ...
    public partial struct WanderSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var entity in entities) {
                ...
                var isMoving = timeLeft > wanderState.Period / 2f;
                animator.SetBool(isMovingID, isMoving);
            }

            foreach (var (transform, wanderState) in SystemAPI.Query<RefRW<LocalTransform>, RefRW<WanderState>>()) {
                ...
                if (timeLeft < 0f) {
                    ...
                    var isMoving = timeLeft > wanderState.ValueRO.Period / 2f;

                    if (isMoving) { transform.ValueRW.Position += transform.ValueRO.Forward() * movement; }
                }
            }
        }
    }
