...
namespace Graphical.TextureUpdate {
    public class TextureUpdate : MonoBehaviour {
        public enum UpdateType { MainThread, MainThreadBurst, ParallelBurstJob, VectorizedParallelBurstJob }

        static ProfilerMarker s_MainThreadProfilerMarker = new("TextureUpdate." + UpdateType.MainThread);
        static ProfilerMarker s_MainThreadBurstProfilerMarker = new("TextureUpdate." + UpdateType.MainThreadBurst);
        static ProfilerMarker s_ParallelJobProfilerMarker = new("TextureUpdate." + UpdateType.ParallelBurstJob);
        static ProfilerMarker s_VectorizedParallelJobProfilerMarker = new("TextureUpdate." + UpdateType.VectorizedParallelBurstJob);

        public int2 textureSize = new(256, 256);
            ...
            switch (updateType) {
                ...
                case UpdateType.MainThreadBurst: {
                    using (s_MainThreadBurstProfilerMarker.Auto()) {
                        new BurstJob {
                            raw = raw, phase = phase, size = textureSize, color = color, period = period,
                        }.Run();
                    }
