...
namespace Graphical.PrefabInitializer {
    ...
    public partial struct WireframeRenderingSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            foreach (var (local, world) in SystemAPI.Query<RefRO<WireframeLocalSpace>, DynamicBuffer<WireframeWorldSpace>>()) {
                var vertices = world.Reinterpret<float3>();
                ref var segments = ref local.ValueRO.Blob.Value.Segments;
                for (var i = 0; i < segments.Length; i++) { Debug.DrawLine(vertices[segments[i].x], vertices[segments[i].y]); }
            }
        }
    }
}
