...
namespace Streaming.AssetManagement {
    ...
    public partial struct AssetLoadingSystem : ISystem {
        public void OnUpdate(ref SystemState state) {
            var noLoadingQuery = SystemAPI.QueryBuilder().WithAll<References>().WithNone<Loading, RequestUnload>().Build();
            ...
            for (var index = 0; index < referencesArray.Length; ++index) {
                ...
                if (refs.EntityPrefabReference.IsReferenceValid) {
                    if (loading.EntityPrefab == Entity.Null) {
                        loading.EntityPrefab = SceneSystem.LoadPrefabAsync(state.WorldUnmanaged, refs.EntityPrefabReference);
                    }
                ...
                if (!loading.GameObjectScene.IsValid() && refs.GameObjectSceneReference.IsReferenceValid) {
                    loading.GameObjectScene = refs.GameObjectSceneReference.LoadAsync(
                        new ContentSceneParameters {
                            autoIntegrate = true, loadSceneMode = LoadSceneMode.Additive, localPhysicsMode = LocalPhysicsMode.None,
                        }
                    );
                }

                // Load GameObject Prefab
                if (loading.GameObjectPrefabInstance == null && refs.GameObjectPrefabReference.IsReferenceValid) {
                    ...
                    if (refs.GameObjectPrefabReference.LoadingStatus == ObjectLoadingStatus.Completed) {
                        loading.GameObjectPrefabInstance = Object.Instantiate(refs.GameObjectPrefabReference.Result);
                    }
                }

                // Load Shader
                if (loading.ShaderInstance == null && refs.ShaderReference.IsReferenceValid) {
                    if (refs.ShaderReference.LoadingStatus == ObjectLoadingStatus.None) { refs.ShaderReference.LoadAsync(); }
                ...
                if (loading.MeshGameObjectInstance == null && refs.MeshReference.IsReferenceValid) {
                    if (refs.MeshReference.LoadingStatus == ObjectLoadingStatus.None) { refs.MeshReference.LoadAsync(); }
                    else if (refs.MeshReference.LoadingStatus == ObjectLoadingStatus.Completed && loading.ShaderInstance) {
                ...
                if (loading.MaterialGameObjectInstance == null && refs.MaterialReference.IsReferenceValid) {
                    if (refs.MaterialReference.LoadingStatus == ObjectLoadingStatus.None) { refs.MaterialReference.LoadAsync(); }
                ...
                if (loading.TextureGameObjectInstance == null && refs.TextureReference.IsReferenceValid) {
                    if (refs.TextureReference.LoadingStatus == ObjectLoadingStatus.None) { refs.TextureReference.LoadAsync(); }
                    else if (refs.TextureReference.LoadingStatus == ObjectLoadingStatus.Completed && loading.ShaderInstance) {
                        // Create an object to display the loaded Texture
        ...
        public GameObject CreateObjectWithMesh(Mesh mesh, Shader shader, string name, float3 position, quaternion rotation) {
