...
namespace Streaming.RuntimeContentManager {
    ...
    [WorldSystemFilter(WorldSystemFilterFlags.Default | WorldSystemFilterFlags.Editor), UpdateInGroup(typeof(SimulationSystemGroup)),
     RequireMatchingQueriesForUpdate]
    public partial struct DecorationVisibilitySystem : ISystem {
        public void OnUpdate(ref SystemState state) {
            new DecorationVisibilityJob {
                CamPos = Camera.main.transform.position, LoadRadius = Camera.main.farClipPlane, CamForward = Camera.main.transform.forward,
            }.ScheduleParallel();
        }
    }
