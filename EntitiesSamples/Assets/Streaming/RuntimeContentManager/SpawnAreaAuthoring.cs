    ...
    public class SpawnAreaAuthoring : MonoBehaviour {
        ...
        class Baker : Baker<SpawnAreaAuthoring> {
            public override void Bake(SpawnAreaAuthoring authoring) {
                ...
                if (collider == null || authoring.meshes.Count == 0 || authoring.materials.Count == 0) { return; }
                ...
                for (var z = bounds.min.z; z < bounds.max.z; z += authoring.spacing) {
                    for (var y = bounds.min.y; y < bounds.max.y; y += authoring.spacing) {
                        for (var x = bounds.min.x; x < bounds.max.x; x += authoring.spacing) {
                            if (math.length(new float2(x, y)) < 10) { continue; }
                            ...
                            AddComponent(
                                entity,
                                new DecorationVisualComponentData {
                            ...
                            AddComponent(entity, new LocalToWorld { Value = new float4x4(quaternion.identity, new float3(x, y, z)) });
                        }
                    }
                }
            }
