...
namespace Streaming.SceneManagement.SectionLoading {
    public partial struct UISystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            if (SectionUI.Singleton == null) { return; }
            ...
            if (entities.Length <= 0 || !state.EntityManager.HasBuffer<ResolvedSectionEntity>(entities[0])) { return; }
            ...
            for (var index = 0; index < sectionEntities.Length; ++index) {
                ...
                var disabled = state.EntityManager.HasComponent<DisableSceneResolveAndLoad>(entity);
            ...
            if (ui.GetAction(out var rowIndex)) {
                ...
                else { state.EntityManager.AddComponent<RequestSceneLoaded>(entity); }
            }
        }
    }
}
