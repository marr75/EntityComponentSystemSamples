...
namespace Streaming.SceneManagement.SceneState {
    [RequireMatchingQueriesForUpdate, UpdateInGroup(typeof(FixedStepSimulationSystemGroup))]
    public partial struct SceneStateSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<SceneReference>(); }
            ...
            for (var index = 0; index < scenes.Length; ++index) {
                ...
                switch (scene.LoadingAction) {
                    ...
                    case LoadingAction.LoadMeta: {
                        var loadParam = new SceneSystem.LoadParameters { AutoLoad = scene.LoadingAction == LoadingAction.LoadAll };
                        if (scene.EntityScene == default) {
                            scene.EntityScene = SceneSystem.LoadSceneAsync(state.WorldUnmanaged, scene.SceneAsset, loadParam);
                        }
                        else { SceneSystem.LoadSceneAsync(state.WorldUnmanaged, scene.EntityScene, loadParam); }

                        break;
                    }
                    case LoadingAction.UnloadAll: {
                        SceneSystem.UnloadScene(state.WorldUnmanaged, scene.EntityScene, SceneSystem.UnloadParameters.DestroyMetaEntities);
