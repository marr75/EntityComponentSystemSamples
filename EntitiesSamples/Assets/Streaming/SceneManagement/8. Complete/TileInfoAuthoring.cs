    ...
    public class TileInfoAuthoring : MonoBehaviour {
        ...
        public class Baker : Baker<TileInfoAuthoring> {
            public override void Bake(TileInfoAuthoring authoring) {
                ...
                var tileTemplates = new List<TileTemplate>(authoring.tileTemplates.Count);
                var sceneReferences = new List<EntitySceneReference>(authoring.tileTemplates.Count);
                ...
                var min = math.min(authoring.minBoundary, authoring.maxBoundary);
                var max = math.max(authoring.minBoundary, authoring.maxBoundary);
                ...
                for (var x = min.x; x <= max.x; ++x) {
                    for (var y = min.y; y <= max.y; ++y) {
                        var selectedTile = random.NextInt(0, tileTemplates.Count);
