...
namespace Streaming.SceneManagement.CompleteSample {
    ...
    partial struct TileLoadingSystem : ISystem {
        ...
        public void OnCreate(ref SystemState state) {
            loadComponentTypeSet = new ComponentTypeSet(
                ComponentType.ReadWrite<RequiresPostLoadCommandBuffer>(),
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            var priorityLoadList = new NativeList<LoadableTile>(tileEntities.Length, Allocator.Temp);

            // Find all the scenes that should be loaded/unloaded based on the distances to relevant entities
            for (var index = 0; index < tileInfos.Length; ++index) {
                if (tileDistances[index] < tileInfos[index].LoadingDistanceSq) {
                    priorityLoadList.Add(new LoadableTile { TileIndex = index, DistanceSq = tileDistances[index] });
                }
                    ...
                    if (state.EntityManager.HasComponent<SubsceneEntity>(tileEntities[index])) {
                        // We unload the scene
                        var complexSceneSubsceneEntity = state.EntityManager.GetComponentData<SubsceneEntity>(tileEntities[index]);
                        SceneSystem.UnloadScene(
                            state.WorldUnmanaged,
                            complexSceneSubsceneEntity.Value,
            ...
            // Load
            var loading = 0;
            var maxToLoad = 4; // limit how many we load at one time
            foreach (var loadEntry in priorityLoadList) {
                var tileIndex = loadEntry.TileIndex;
                if (!state.EntityManager.HasComponent<SubsceneEntity>(tileEntities[tileIndex])) {
                    ...
                    var sceneEntity = SceneSystem.LoadSceneAsync(
                        state.WorldUnmanaged,
                        tileInfos[tileIndex].Scene,
                        new SceneSystem.LoadParameters() { Flags = SceneLoadFlags.NewInstance | SceneLoadFlags.DisableAutoLoad }
                    );

                    var center = new float3(tileInfos[tileIndex].Position.x, 0f, tileInfos[tileIndex].Position.y);
                    ...
                    state.EntityManager.SetComponentData(
                        sceneEntity,
                        new RequiresPostLoadCommandBuffer { Position = center, Rotation = tileInfos[tileIndex].Rotation }
                    );

                    // We also store the center of the tile, to make it accessible for distance checks on it during the section loading
                    state.EntityManager.SetComponentData(sceneEntity, new TileEntity { Value = tileEntities[tileIndex] });

                    // We store the scene entity to handle the unload later
                    state.EntityManager.AddComponentData(tileEntities[tileIndex], new SubsceneEntity { Value = sceneEntity });
