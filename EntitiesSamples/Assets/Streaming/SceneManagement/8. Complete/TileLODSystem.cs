...
namespace Streaming.SceneManagement.CompleteSample {
    ...
    partial struct TileLODSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            var noTileEntityQuery = SystemAPI.QueryBuilder().WithAll<TileLODRange, SceneEntityReference>().WithNone<TileEntity>().Build();
            ...
            for (var index = 0; index < sceneEntityReferences.Length; ++index) {
                var tileEntity = state.EntityManager.GetComponentData<TileEntity>(sceneEntityReferences[index].SceneEntity);
            ...
            var sectionQuery = SystemAPI.QueryBuilder().WithAll<TileLODRange, TileEntity, SceneSectionData>().Build();
            ...
            for (var index = 0; index < tileEntities.Length; ++index) {
                var distanceComponent = state.EntityManager.GetComponentData<DistanceToRelevant>(tileEntities[index].Value);
                var distanceValueSq = distanceComponent.DistanceSq;
                if (distanceValueSq >= lodRanges[index].LowerRadiusSq && distanceValueSq < lodRanges[index].HigherRadiusSq) {
                    toLoad.Add(sectionEntities[index]);
                }
            }

            // Cache the streaming state of the section
            var streamingStateLookup = new NativeHashMap<Entity, SceneSystem.SectionStreamingState>(1, Allocator.Temp);
            ...
            foreach (var sectionEntity in sectionEntities) {
                ...
                if (toLoad.Contains(sectionEntity)) {
                    ...
                    var sceneEntityReference = state.EntityManager.GetComponentData<SceneEntityReference>(sectionEntity);
                    var sceneSectionEntities = state.EntityManager.GetBuffer<ResolvedSectionEntity>(sceneEntityReference.SceneEntity, true);

                    var sectionsLoaded = 0;
                    for (var index = 1; index < sceneSectionEntities.Length; ++index) {
                        var neighbourSectionState = streamingStateLookup[sceneSectionEntities[index].SectionEntity];
                        if (neighbourSectionState == SceneSystem.SectionStreamingState.Loaded) { ++sectionsLoaded; }
                    }

                    // Unload if there is at least one other section loaded
                    if (sectionsLoaded > 1) { state.EntityManager.RemoveComponent<RequestSceneLoaded>(sectionEntity); }
                }
            }
        }
    }
