...
namespace Streaming.SceneManagement.StreamingVolume {
    partial struct VolumeSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<Relevant>(); }

        [BurstCompile]
        public void OnUpdate(ref SystemState state) {
            // Check which volumes contain any relevant entities.
            var activeVolumes = new NativeHashSet<Entity>(100, Allocator.Temp);
            ...
            foreach (var (transform, volume, volumeEntity) in SystemAPI.Query<RefRO<LocalToWorld>, RefRO<Volume>>().WithEntityAccess()) {
            ...
            var toLoadList = new NativeList<(Entity, LevelInfo)>(10, Allocator.Temp);
            var toUnloadList = new NativeList<(Entity, LevelInfo)>(10, Allocator.Temp);

            foreach (var (volumes, levelInfo, entity) in
                SystemAPI.Query<DynamicBuffer<VolumeBuffer>, RefRW<LevelInfo>>().WithEntityAccess()) {
                ...
                if (shouldLoad && levelInfo.ValueRW.runtimeEntity == Entity.Null) { toLoadList.Add((entity, levelInfo.ValueRW)); }
                else if (!shouldLoad && levelInfo.ValueRW.runtimeEntity != Entity.Null) { toUnloadList.Add((entity, levelInfo.ValueRW)); }
            }
