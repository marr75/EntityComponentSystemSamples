    ...
    public class VolumeAuthoring : MonoBehaviour {
        public class Baker : Baker<VolumeAuthoring> {
            public override void Bake(VolumeAuthoring authoring) {
                ...
                AddComponent(entity, new Volume { Scale = GetComponent<Transform>().localScale });

                AddComponent(entity, new StreamingGO { InstanceID = authoring.gameObject.GetInstanceID() });
            }
        }
    }
