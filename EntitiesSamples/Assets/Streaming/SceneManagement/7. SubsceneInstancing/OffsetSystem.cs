...
namespace Streaming.SceneManagement.SubsceneInstancing {
    ...
    public partial struct OffsetSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<Offset>(); }
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var offset in offsets) {
                ...
                foreach (var transform in SystemAPI.Query<RefRW<LocalTransform>>()) { transform.ValueRW.Position += offset.Value; }

                // Apply the offset to all non-dynamic entities
                var offsetMatrix = float4x4.Translate(offset.Value);
                foreach (var transform in SystemAPI.Query<RefRW<LocalToWorld>>().WithNone<LocalTransform>()) {
                    transform.ValueRW.Value = math.mul(offsetMatrix, transform.ValueRW.Value);
                }

                // Apply the offset to the center of oscillation
                foreach (var oscillating in SystemAPI.Query<RefRW<Oscillating>>()) { oscillating.ValueRW.Center += offset.Value; }
            }
        }
    }
}
