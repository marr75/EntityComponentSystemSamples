...
namespace Streaming.SceneManagement.SubsceneInstancing {
    ...
    public partial struct SpawnSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<Grid>(); }
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            for (var index = 0; index < grids.Length; index += 1) {
                ...
                var centerOffset = -((grids[index].Size - 1) * grids[index].Spacing);
                centerOffset /= 2f;

                // Create the subscene instances.
                for (var i = 0; i < grids[index].Size; ++i) {
                    for (var j = 0; j < grids[index].Size; ++j) {
                        var sceneEntity = SceneSystem.LoadSceneAsync(state.WorldUnmanaged, grids[index].Scene, loadParameters);
                        ...
                        buf.CommandBuffer.AddComponent(
                            postLoadEntity,
                            new Offset {
                                Value = new float3(
