...
namespace Streaming.PrefabAndSceneReferences {
    [RequireMatchingQueriesForUpdate, WorldSystemFilter(WorldSystemFilterFlags.Default | WorldSystemFilterFlags.Editor)]
    public partial struct LoadingSystem : ISystem {
        public void OnUpdate(ref SystemState state) {
            ...
            for (var i = 0; i < entities.Length; i++) {
                ...
                ecb.AddComponent(entity, new CleanupSceneReference() { SceneToUnload = sceneEntity });
                ecb.RemoveComponent<SceneReference>(entity);
            }

            // Load the PrefabReferences
            foreach (var (prefabRef, entity) in SystemAPI.Query<RefRO<PrefabReference>>()
                .WithNone<RequestEntityPrefabLoaded>()
                .WithEntityAccess()) { ecb.AddComponent(entity, new RequestEntityPrefabLoaded { Prefab = prefabRef.ValueRO.Value }); }

            // Instantiate the PrefabReferences
            foreach (var (loadedPrefab, entity) in
                SystemAPI.Query<RefRO<PrefabLoadResult>>().WithAll<PrefabReference>().WithEntityAccess()) {
                var prefabEntity = ecb.Instantiate(loadedPrefab.ValueRO.PrefabRoot);
                ecb.AddComponent(entity, new CleanupPrefabReference() { PrefabToUnload = prefabEntity });
            ...
            foreach (var (prefabRef, entity) in SystemAPI.Query<RefRO<CleanupPrefabReference>>().WithNone<SceneTag>().WithEntityAccess()) {
