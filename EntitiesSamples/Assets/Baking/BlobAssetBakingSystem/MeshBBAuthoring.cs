    ...
    public class MeshBBAuthoring : MonoBehaviour {
        ...
        class Baker : Baker<MeshBBAuthoring> {
            public override void Bake(MeshBBAuthoring authoring) {
                ...
                AssetDatabase.TryGetGUIDAndLocalFileIdentifier(mesh.GetInstanceID(), out var guid, out var localId);
                ...
                if (IsBuiltin(new GUID(guid))) { hash = new Hash128((uint)localId, (uint)authoring.MeshScale.GetHashCode(), 0, 0); }
                else if (EditorUtility.IsPersistent(mesh)) { hash = AssetDatabase.GetAssetDependencyHash(assetPath); }
                else { Debug.LogError("This sample does not support procedural meshes stored in the scene"); }
                ...
                AddComponent(entity, new RawMesh() { MeshScale = authoring.MeshScale, Hash = hash });

                // Add the Component that will hold the BlobAssetReference later 
                AddComponent(entity, new MeshBB());
            }

            public static GUID UnityEditorResources = new("0000000000000000d000000000000000");
            public static GUID UnityBuiltinResources = new("0000000000000000e000000000000000");
            public static GUID UnityBuiltinExtraResources = new("0000000000000000f000000000000000");
