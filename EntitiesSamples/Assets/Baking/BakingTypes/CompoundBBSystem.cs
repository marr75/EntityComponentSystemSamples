...
namespace Baking.BakingTypes {
    ...
    public partial struct CompoundBBSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            var missingCleanupQuery = SystemAPI.QueryBuilder().WithAll<BoundingBox>().WithNone<BoundingBoxCleanup>().Build();
            ...
            foreach (var (bb, pp) in SystemAPI.Query<RefRO<BoundingBox>, RefRW<BoundingBoxCleanup>>().WithAll<Changes>()) {
            ...
            foreach (var pp in SystemAPI.Query<RefRO<BoundingBoxCleanup>>().WithNone<BoundingBox>()) {
                var previousParent = pp.ValueRO.PreviousParent;
                if (previousParent != Entity.Null) { changedCBBs.Add(previousParent); }
            }

            // Destroyed entities are kept alive by their cleanup component, so they have to be explicitly removed.
            var removedEntities = SystemAPI.QueryBuilder().WithAll<BoundingBoxCleanup>().WithNone<BoundingBox>().Build();
            ...
            foreach (var parent in changedCBBs) {
                SystemAPI.SetComponent(
                    parent,
                    new CompoundBBComponent() { MinBBVertex = new float3(float.MaxValue), MaxBBVertex = new float3(float.MinValue) }
                );
            }
            ...
            foreach (var bb in SystemAPI.Query<RefRO<BoundingBox>>()) {
                var parent = bb.ValueRO.Parent;
                if (!changedCBBs.Contains(parent)) { continue; }
