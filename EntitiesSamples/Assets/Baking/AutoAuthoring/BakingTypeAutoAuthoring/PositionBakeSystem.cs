...
namespace Baking.AutoAuthoring.BakingTypeAutoAuthoring {
    ...
    public partial struct PositionBakeSystem : ISystem {
        [BurstCompile]
        public void OnCreate(ref SystemState state) { state.RequireForUpdate<Complex>(); }

        [BurstCompile]
        public void OnUpdate(ref SystemState state) {
            foreach (var (complex, transform) in SystemAPI.Query<RefRO<Complex>, RefRW<LocalTransform>>()
                .WithAll<BakedEntity>()
                .WithOptions(EntityQueryOptions.IncludeDisabledEntities | EntityQueryOptions.IncludePrefab)) {
                transform.ValueRW = LocalTransform.FromPositionRotation(
                    complex.ValueRO.Properties.Position,
